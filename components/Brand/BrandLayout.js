            ...
            <div className='flex justify-between'>
              <Link href='../'>
                <a className='hover:opacity-50 transition-opacity duration-150 inline-flex items-end gap-2'>
                  ...
                  <div className='font-headings text-xl uppercase not-italic leading-none text-ui-300 pb-1'>
                    Brand
                  </div>
                </a>
              </Link>
              ...
              <div className={`${filter ? '' : 'mb-4'}`} key={itemIndex}>
                ...
                <div className={`${router.pathname.includes(item.url) ? 'is-active' : ''}`}>
                  <div className={`${filter ? '' : 'border-t border-ui-700/50'}`}>
                    <BrandMainNav filter={filter} items={item.pages} style='secondary' rounded='true' onClick={executeScroll.bind(this)} />
                  </div>
                </div>
              </div>
            ))}
