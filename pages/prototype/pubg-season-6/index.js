    ...
    <section 
      ...
      <div className='container mx-auto max-w-xl flex flex-col relative pb-8'>
        ...
        <div className='relative z-10 max-w-[100vw] px-2 lg:px-0'>
          <iframe className='mx-auto max-w-[100%] block aspect-video h-auto rounded-2xl' src={`https://player.twitch.tv/?video=1465753668&parent=gloot-framework.netlify.app`} allowFullScreen={true} height={676} width={1082}></iframe>
        </div>
        ...
        <div className='mx-auto max-w-xl flex overflow-x-auto scrollbar-desktop justify-start gap-4 bg-gradient-to-b from-mono-900/50 to-ui-900 px-4 py-8 sm:rounded-2xl'>
          ...
          <div className='flex-1 flex justify-center gap-4 lg:gap-8 '>
            ...
            {TournamentLeaderboard.map((item, itemIndex) => (
              <>
                <div onClick={() => switchLeaderboard(itemIndex, 0, 0, 0)} className={`surface w-56 h-44 lg:w-80 lg:h-60 flex flex-col items-between justify-between rounded-lg items-center text-center transition-all duration-300 ease-in-out hover:-translate-y-2 cursor-pointer ease-in-out ${SelectedTournament === itemIndex ? 'surface-main' : 'opacity-50'}`}>
