export default async function handler(req, res) {
  res.status(200).json({
    key: '4945142093447168',
    username: 'harrygloot',
    currency: 'USD',
    email: 'harris@gloot.com',
    profilePicture:
      'https://lh3.googleusercontent.com/lQ5a_WTp-lpKidTPC-6zC2EFavOyp1IQ-bifIOcSWlZpceIv3tVSnpWdigNAd8X1sxctGDuDeKJaotT2Ztr326Gr78yvP9xmKypK=s512',
    profilePictureMap: {
      small:
        'https://lh3.googleusercontent.com/lQ5a_WTp-lpKidTPC-6zC2EFavOyp1IQ-bifIOcSWlZpceIv3tVSnpWdigNAd8X1sxctGDuDeKJaotT2Ztr326Gr78yvP9xmKypK=s50',
      medium:
        'https://lh3.googleusercontent.com/lQ5a_WTp-lpKidTPC-6zC2EFavOyp1IQ-bifIOcSWlZpceIv3tVSnpWdigNAd8X1sxctGDuDeKJaotT2Ztr326Gr78yvP9xmKypK=s80',
      large:
        'https://lh3.googleusercontent.com/lQ5a_WTp-lpKidTPC-6zC2EFavOyp1IQ-bifIOcSWlZpceIv3tVSnpWdigNAd8X1sxctGDuDeKJaotT2Ztr326Gr78yvP9xmKypK=s130',
      gigantic:
        'https://lh3.googleusercontent.com/lQ5a_WTp-lpKidTPC-6zC2EFavOyp1IQ-bifIOcSWlZpceIv3tVSnpWdigNAd8X1sxctGDuDeKJaotT2Ztr326Gr78yvP9xmKypK=s512',
    },
    registrationDate: 1574085679240,
    birthDate: 347155200000,
    geoCountry: 'USA',
    preferredCountry: 'SWE',
    twitchUsername: null,
    twitterUsername: null,
    roles: [
      'FEATURE_USER_PAYMENTS_EDITOR',
      'GLL_FEATURE_REMOVE_TOURNAMENT_MATCHES',
      'ID',
      'FEATURE_POT_DISTRIBUTION_REPORT_SCORE',
      'FEATURE_CMS',
      'GLL_NORMAL_ADMIN',
      'FEATURE_ENROLLMENT_VIEWER',
      'FEATURE_MISSION_EDIT',
      'SUBSCRIPTIONS_EDITOR',
      'SERVICE_ACCOUNT_VIEWER',
      'FEATURE_ADJUST_FUNDS',
      'FEATURE_SLS_VIEW',
      'FEATURE_INSERT_NOTIFICATIONS',
      'FEATURE_VOUCHER_VIEW_PROTECTED',
      'GAME_MANAGER',
      'FEATURE_RESERVE_MONEY',
      'GLL_SCORE_ADMIN',
      'FEATURE_USER_RANK_EDITOR',
      'FEATURE_WITHDRAW',
      'GLL_FEATURE_ADD_TOURNAMENT_MATCHES',
      'FEATURE_POT_DISTRIBUTION_VIEW_TEMPLATE',
      'FEATURE_SUBSCRIPTION_STATE_CHANGER',
      'FEATURE_TERMS_AND_CONDITIONS_EDIT',
      'FEATURE_OAUTH_VIEWER',
      'FEATURE_GENERATE_SERVICE_ACCOUNT_JWT',
      'FEATURE_MATCH_DELETE',
      'GLL_FEATURE_EDIT_FEATURE_TOGGLE',
      'FEATURE_CHALLENGE_APPROVE',
      'FEATURE_COMPLIANCE_VIEW',
      'FEATURE_CREATE_SERVICE_ACCOUNT',
      'FEATURE_MOBILE_GAME_STATUS',
      'FEATURE_ADJUST_FUNDS_APPROVAL_VIEW',
      'FEATURE_GAME_STATUS_EDIT',
      'FEATURE_LEAGUE_EDITOR',
      'FEATURE_REWARD_LADDER_EDIT',
      'CMS_EDITOR',
      'FEATURE_ROLES',
      'FEATURE_SLS_EDIT',
      'GNOG_CHALLENGE_APPROVER',
      'GLL_FEATURE_ADD_ROUNDS',
      'GLL_HEAD_ADMIN',
      'FEATURE_EDIT_SERVICE_ACCOUNT',
      'FEATURE_POT_DISTRIBUTION_CREATE',
      'FEATURE_VOUCHER_VIEW',
      'EMAIL_ADMIN',
      'USER',
      'FEATURE_BACKUP',
      'FEATURE_CONVERSION_TABLE_EDITOR',
      'FEATURE_VIEW_PROTECTED_REFRESH_DATA',
      'FEATURE_API',
      'FEATURE_EDIT_EMAIL_TEMPLATES',
      'FEATURE_CHALLENGE_EDIT',
      'FEATURE_VIEW_SUBSCRIPTIONS',
      'FEATURE_VIEW_SERVICE_ACCOUNT',
      'GLL_FEATURE_CHANGE_MATCH_STATE',
      'FEATURE_COMPLIANCE_EDIT',
      'FEATURE_BLOCK_USER',
      'FEATURE_SEND_EMAILS',
      'GLL_FEATURE_EARLY_ACCESS',
      'FEATURE_GAME_CONFIGURATION_EDIT',
      'FEATURE_COIN_GIVEAWAY',
      'DAILY_SCHEDULER',
      'FEATURE_PAYMENT_CALLBACK',
      'FEATURE_ADJUST_FUNDS_SKIP_APPROVAL',
      'FEATURE_COIN_BUNDLE_EDITOR',
      'FEATURE_GDPR_SCRAMBLE_USER',
      'FEATURE_LEAGUE_REPORTER',
      'FEATURE_COIN_FUNDS_EDITOR',
      'FEATURE_COIN_FUNDS_VIEWER',
      'GLL_FEATURE_REMOVE_ROUNDS',
      'COIN_ADMIN',
      'FEATURE_LEADERBOARD_AUDIT',
      'SERVICE_ACCOUNT_EDITOR',
      'FEATURE_COIN_BUNDLE_PURCHASE',
      'FEATURE_SEGMENT_TRACKING',
      'FEATURE_TOURNAMENT_ADMIN',
      'FEATURE_SETUP_SUBSCRIPTIONS',
      'FEATURE_CREATE_PAYMENT_PORTAL_TOKEN',
      'GLL_SERVER',
      'FEATURE_GAME_ADMIN',
      'FEATURE_CHALLENGE_CREATE_CONTENT',
      'FEATURE_COIN_FUNDS_ADMIN',
      'FEATURE_VOUCHER_RESERVE',
      'SERVER_REPORT_SCORE',
      'CASHIER',
      'FEATURE_VIEW_BLOCKS',
      'FEATURE_ADJUST_FUNDS_APPROVAL_EDIT',
      'ADJUST_FUNDS_APPROVER',
      'FEATURE_DAILY_SCHEDULE',
      'SERVER_REMOTE_LOGIN',
      'SUBSCRIPTIONS_VIEWER',
      'FEATURE_VOUCHER_APPROVE',
      'FEATURE_USERINFO_EDIT',
      'FEATURE_CONVERSION_TABLE_VIEWER',
      'FEATURE_SLS_REPORT',
      'FEATURE_USERINFO_VIEW',
      'FCM_ADMIN',
      'GLL_FEATURE_REGISTER_TEAMS_TO_MATCHES',
      'FEATURE_BUDGET_APPROVER',
      'FEATURE_COIN_VIEWER',
      'FEATURE_COIN_WALLET_VIEWER',
      'SUPPORT',
      'FEATURE_COIN_RESERVATION',
      'FEATURE_VOUCHER_CREATE',
      'SUPER_USER',
      'FEATURE_USER_MANAGEMENT',
      'FEATURE_XSOLLA_VIEWER',
      'FEATURE_CORE_SETTINGS',
      'FEATURE_KYC_VIEWER',
      'FEATURE_STATS_PROPERTIES_EDIT',
      'FEATURE_SET_PASSWORD',
      'FEATURE_VIEW_EMAIL_TEMPLATES',
      'FEATURE_KYC_EDITOR',
      'FEATURE_CHANGE_PASSWORD_WITHOUT_OLD_PASS',
      'COIN_VIEWER',
      'FEATURE_POT_DISTRIBUTION_EDIT_TEMPLATE',
      'SERVER_EMAIL',
      'FEATURE_HIJACK_USER',
      'GNOG_CHALLENGE_CONTENT_CREATOR',
      'FEATURE_SEND_WALLET_LINK_EMAIL',
      'GLL_FEATURE_CHANGE_ROUND_STATE',
      'GNOG_MOBILE_ADMIN',
      'FEATURE_ACHIEVEMENT_EDITOR',
      'FEATURE_WALLET_EVENT_VIEWER',
      'FEATURE_OAUTH_EDITOR',
      'FEATURE_CHALLENGE_VIEW',
      'GLL_FEATURE_REPORT_SCORE',
      'SUBSCRIPTIONS_ADMIN',
      'DEVELOPER',
      'SERVER_SUBSCRIPTIONS',
      'FEATURE_COIN_WALLET_EDITOR',
      'GNOG_ADMIN',
      'GLL_FEATURE_EDIT_TOURNAMENT',
      'FEATURE_COIN_EDITOR',
      'FEATURE_BONUS_ADMIN',
      'GLL_FEATURE_VERIFY_LEADERBOARD',
      'FEATURE_COIN_BUNDLE_VIEWER',
      'FEATURE_VIEW_PENDING_PARTY_LOG',
    ],
    depositRanges: [10, 20, 30],
    depositRangesFormatted: ['$10.00', '$20.00', '$30.00'],
    hasInstalledClient: true,
    info: {
      games: [
        {
          assets: {
            color: '#D03B34',
            cover:
              'https://res.cloudinary.com/gloot/image/upload/f_auto/v1622115586/GNOG%20assets/Games/apex_legends/cover.png',
            cover_dark:
              'https://res.cloudinary.com/gloot/image/upload/f_auto/v1633936363/GNOG%20assets/Games/apex_legends/cover_dark.png',
            game_picker:
              'https://res.cloudinary.com/gloot/image/upload/f_auto/v1632130039/GNOG%20assets/Games/apex_legends/game_picker.png',
            hero: 'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619016839/GNOG%20assets/Games/apex_legends/hero.jpg',
            logo: {
              primary_color:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619015845/GNOG%20assets/Games/apex_legends/logo_primary_color.png',
              primary_white:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619015830/GNOG%20assets/Games/apex_legends/logo_primary_white.png',
              secondary_color:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619078980/GNOG%20assets/Games/apex_legends/logo_secondary_color.png',
              secondary_white:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619078670/GNOG%20assets/Games/apex_legends/logo_secondary_white.png',
            },
          },
          id: '787864400',
          maintenance: false,
          metaDescription:
            'Stryda is an esports platform. Play APEX Legend for fun and glory. No queue time. No subscription. Play free or compete for prize pool. Start the fun now.',
          name: 'Apex Legends',
          overwolfGameId: '21566',
          shortName: 'Apex Legends',
          slug: 'apex_legends',
          supportsTournaments: true,
          twitchGameName: 'Apex Legends',
          userGameNameProperty: 'apexNick',
          userGamePlaceholder: 'Write your Apex in-game nickname',
          weight: 3000,
        },
        {
          assets: {
            color: '#F2A900',
            cover: 'https://res.cloudinary.com/gloot/image/upload/f_auto/v1643719500/GNOG%20assets/Games/pubg/cover1.jpg',
            cover_dark:
              'https://res.cloudinary.com/gloot/image/upload/f_auto/v1633936853/GNOG%20assets/Games/pubg/cover_dark.png',
            game_picker:
              'https://res.cloudinary.com/gloot/image/upload/f_auto/v1632130437/GNOG%20assets/Games/pubg/game_picker.png',
            hero: '',
            logo: {
              primary_color:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619016454/GNOG%20assets/Games/pubg/logo_primary_color.png',
              primary_white:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619016443/GNOG%20assets/Games/pubg/logo_primary_white.png',
              secondary_color:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619080759/GNOG%20assets/Games/pubg/logo_secondary_color.png',
              secondary_white:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1642068586/GNOG%20assets/Games/pubg/logo_secondary_whitev2.png',
            },
          },
          id: '3966731725',
          maintenance: false,
          name: 'PUBG BATTLEGROUNDS',
          overwolfGameId: '10906',
          shortName: 'PUBG',
          slug: 'pubg',
          supportsTournaments: true,
          twitchGameName: 'PUBG BATTLEGROUNDS',
          userGameNameProperty: 'pubgNick',
          userGamePlaceholder: 'Write your PUBG in-game nickname',
          weight: 2950,
        },
        {
          assets: {
            color: '#E45357',
            cover: 'https://res.cloudinary.com/gloot/image/upload/f_auto/v1622115451/GNOG%20assets/Games/valorant/cover.png',
            cover_dark:
              'https://res.cloudinary.com/gloot/image/upload/f_auto/v1633936936/GNOG%20assets/Games/valorant/cover_dark.png',
            game_picker:
              'https://res.cloudinary.com/gloot/image/upload/f_auto/v1632130606/GNOG%20assets/Games/valorant/game_picker.png',
            hero: 'https://res.cloudinary.com/gloot/image/upload/f_auto/v1622190427/GNOG%20assets/Games/valorant/hero.jpg',
            logo: {
              primary_color:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619014308/GNOG%20assets/Games/valorant/logo_primary_color.png',
              primary_white:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619014356/GNOG%20assets/Games/valorant/logo_primary_white.png',
              secondary_color:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619081223/GNOG%20assets/Games/valorant/logo_secondary_color.png',
              secondary_white:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619081202/GNOG%20assets/Games/valorant/logo_secondary_white.png',
            },
          },
          id: '1892449706',
          maintenance: false,
          metaDescription:
            'Stryda is an esports platform. Play Valorant for fun and glory. No queue time. No subscription. Play free or compete for prize pool. Start the fun now.',
          name: 'Valorant',
          overwolfGameId: '21640',
          shortName: 'Valorant',
          slug: 'valorant',
          supportsTournaments: true,
          twitchGameName: 'VALORANT',
          userGameNameProperty: 'riotId',
          userGamePlaceholder: 'Write your Riot ID#Tagline e.g. PlayeR#1611',
          weight: 2800,
        },
        {
          assets: {
            color: '#F98100',
            cover: 'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619009178/GNOG%20assets/Games/csgo/cover.png',
            cover_dark:
              'https://res.cloudinary.com/gloot/image/upload/f_auto/v1633936412/GNOG%20assets/Games/csgo/cover_dark.png',
            game_picker:
              'https://res.cloudinary.com/gloot/image/upload/f_auto/v1632130064/GNOG%20assets/Games/csgo/game_picker.png',
            hero: 'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619009935/GNOG%20assets/Games/csgo/hero.jpg',
            logo: {
              primary_color:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619013513/GNOG%20assets/Games/csgo/logo_primary_color.png',
              primary_white:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619013517/GNOG%20assets/Games/csgo/logo_primary_white.png',
              secondary_color:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619075288/GNOG%20assets/Games/csgo/logo_secondary_color.png',
              secondary_white:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619075262/GNOG%20assets/Games/csgo/logo_secondary_white.png',
            },
          },
          id: '2619660763',
          maintenance: false,
          metaDescription:
            'Stryda is an esports platform. Play Counter-Strike: Global Offensive for fun and glory. No queue time. No subscription. Play free or compete for prize pool. Start the fun now.',
          name: 'Counter-Strike: Global Offensive',
          overwolfGameId: '7764',
          shortName: 'CS:GO',
          slug: 'csgo',
          supportsTournaments: false,
          twitchGameName: 'Counter-Strike: Global Offensive',
          userGameNameProperty: 'csgoNick',
          userGamePlaceholder: 'Write your CS:GO in-game nickname',
          weight: 2700,
        },
        {
          assets: {
            color: '#EF3D36',
            cover: 'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619009254/GNOG%20assets/Games/dota_2/cover.png',
            cover_dark:
              'https://res.cloudinary.com/gloot/image/upload/f_auto/v1633936644/GNOG%20assets/Games/dota_2/cover_dark.png',
            game_picker:
              'https://res.cloudinary.com/gloot/image/upload/f_auto/v1632130084/GNOG%20assets/Games/dota_2/game_picker.png',
            hero: 'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619011943/GNOG%20assets/Games/dota_2/hero.jpg',
            logo: {
              primary_color:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619013651/GNOG%20assets/Games/dota_2/logo_primary_color.png',
              primary_white:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619013663/GNOG%20assets/Games/dota_2/logo_primary_white.png',
              secondary_color:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619079099/GNOG%20assets/Games/dota_2/logo_secondary_color.png',
              secondary_white:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619079078/GNOG%20assets/Games/dota_2/logo_secondary_white.png',
            },
          },
          id: '1440666116',
          maintenance: false,
          metaDescription:
            'Stryda is an esports platform. Play DOTA 2 for fun and glory. No queue time. No subscription. Play free or compete for prize pool. Start the fun now.',
          name: 'DOTA2',
          overwolfGameId: '7314',
          shortName: 'DOTA2',
          slug: 'dota_2',
          supportsTournaments: false,
          twitchGameName: 'Dota 2',
          userGameNameProperty: 'dotaTwoNick',
          userGamePlaceholder: 'Write your Dota 2 in-game nickname',
          weight: 2500,
        },
        {
          assets: {
            color: '#C5954A',
            cover:
              'https://res.cloudinary.com/gloot/image/upload/f_auto/v1622103228/GNOG%20assets/Games/league_of_legends/cover.png',
            cover_dark:
              'https://res.cloudinary.com/gloot/image/upload/f_auto/v1633936699/GNOG%20assets/Games/league_of_legends/cover_dark.png',
            game_picker:
              'https://res.cloudinary.com/gloot/image/upload/f_auto/v1632130397/GNOG%20assets/Games/league_of_legends/game_picker.png',
            hero: '',
            logo: {
              primary_color:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619014180/GNOG%20assets/Games/league_of_legends/logo_primary_color.png',
              primary_white:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619014193/GNOG%20assets/Games/league_of_legends/logo_primary_white.png',
              secondary_color:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619080527/GNOG%20assets/Games/league_of_legends/logo_secondary_color.png',
              secondary_white:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619080523/GNOG%20assets/Games/league_of_legends/logo_secondary_white.png',
            },
          },
          id: '3434118582',
          maintenance: false,
          metaDescription:
            'Stryda is an esports platform. Play League of Legends for fun and glory. No queue time. No subscription. Play free or compete for prize pool. Start the fun now.',
          name: 'League of Legends',
          overwolfGameId: '5426',
          shortName: 'LoL',
          slug: 'league_of_legends',
          supportsTournaments: false,
          twitchGameName: 'League Of Legends',
          userGameNameProperty: 'leagueOfLegendsNick',
          userGamePlaceholder: 'Write your League of Legends in-game nickname',
          weight: 2400,
        },
        {
          assets: {
            color: '#2E71C6',
            cover:
              'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619015677/GNOG%20assets/Games/rocket_league/cover.png',
            cover_dark:
              'https://res.cloudinary.com/gloot/image/upload/f_auto/v1633936893/GNOG%20assets/Games/rocket_league/cover_dark.png',
            game_picker:
              'https://res.cloudinary.com/gloot/image/upload/f_auto/v1632130472/GNOG%20assets/Games/rocket_league/game_picker.png',
            hero: 'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619016970/GNOG%20assets/Games/rocket_league/hero.jpg',
            logo: {
              primary_color:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619015715/GNOG%20assets/Games/rocket_league/logo_primary_color.png',
              primary_white:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619015734/GNOG%20assets/Games/rocket_league/logo_primary_white.png',
              secondary_color:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619081060/GNOG%20assets/Games/rocket_league/logo_secondary_color.png',
              secondary_white:
                'https://res.cloudinary.com/gloot/image/upload/f_auto/v1619081048/GNOG%20assets/Games/rocket_league/logo_secondary_white.png',
            },
          },
          id: '1719436358',
          maintenance: false,
          metaDescription:
            'Stryda is an esports platform. Play Rocket League for fun and glory. No queue time. No subscription. Play free or compete for prize pool. Start the fun now.',
          name: 'Rocket League',
          overwolfGameId: '10798',
          shortName: 'Rocket League',
          slug: 'rocket_league',
          supportsTournaments: false,
          twitchGameName: 'Rocket League',
          userGameNameProperty: 'rocketLeagueNick',
          userGamePlaceholder: 'Write your Rocket League in-game nickname',
          weight: 1900,
        },
      ],
      onboarding: {
        pickGames: true,
        menuTour: true,
        mobile: false,
        desktopClient: false,
      },
      socials: {
        twitch: null,
        youtube: null,
        twitter: null,
      },
      getStarted: {
        hasClaimed: true,
        hasInstalledClient: true,
        hasPlayedLadder: true,
        hasPlayedMission: true,
      },
      legal: {
        toc: null,
      },
    },
    traits: {
      dev_country_segment: 3,
      dev_last_challenge_kills: 0,
      dev_valorant_kill_segments: 'segment-30',
      dev_valorant_kills_7cra: 14,
      dev_valorant_kills_segment: 'segment-10',
      dev_valorant_winrate_7cra: 0,
    },
    termsAndConditionsVersion: 5,
    steamName: null,
    riotId: 'Babayaga',
    apexNick: 'tjaaa',
    pubgNick: null,
    pubgMobileNick: null,
    freefireId: null,
    discordName: null,
    hasPremium: true,
  });
}
